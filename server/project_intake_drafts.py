    ...
    *,
    ...
    conn.execute(
        ...
        (id, owner_user_id, status, document_id, document_version_id,
         schema_version, values_json, fallback_reason, fallback_note,
         completed_project_id, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, 'contract.v1', ?, ?, ?, NULL, ?, ?)
        """,
        (
            draft_id,
            owner_user_id,
    ...
    *,
    ...
    conn.execute(
        ...
        SET status = ?, document_id = ?, document_version_id = ?, values_json = ?,
            fallback_reason = ?, fallback_note = ?, completed_project_id = ?, updated_at = ?
        WHERE id = ? AND owner_user_id = ?
        """,
        (
            status,
...
def _map_row(row):
    item = dict(row)
    try:
        values = json.loads(item.get("values_json") or "{}")
