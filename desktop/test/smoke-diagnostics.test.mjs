...
test('smoke runner waits for closed output pipes and wraps scenario assertions', () => {
  ...
  const timeoutStart = completionBlock.indexOf('timeout = setTimeout')
  const timeoutEnd = completionBlock.indexOf("child.once('error'", timeoutStart)
  const timeoutBlock = completionBlock.slice(timeoutStart, timeoutEnd)
